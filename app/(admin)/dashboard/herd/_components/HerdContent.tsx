...
export default function HerdContent({ initialCows }: HerdContentProps) {
  ...
  const filteredCows = useMemo(() => {
    return initialCows.filter(cow => {
      const nameMatch = (cow.name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (cow.deviceId || '').toString().toLowerCase().includes(searchQuery.toLowerCase());
      const statusMatch = statusFilter === 'All' || cow.healthStatus === statusFilter;
  ...
  const stats = {
    ...
    healthy: initialCows.filter(c => c.healthStatus === 'Healthy').length,
    warning: initialCows.filter(c => c.healthStatus === 'Warning' || c.healthStatus === 'Unhealthy').length,
  };
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center justify-between bg-white/60 p-6 rounded-[2.5rem] border border-white backdrop-blur-3xl shadow-xl">
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
          {['All', 'Healthy', 'Warning', 'Critical'].map(healthStatus => (
            <button
              key={healthStatus}
              onClick={() => setStatusFilter(healthStatus)}
              className={`h-14 px-8 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all shadow-sm ${statusFilter === healthStatus ? 'bg-[#023b26] text-white shadow-emerald-900/20' : 'bg-white text-gray-400 border border-gray-100 hover:bg-gray-50'}`}
            >
              {healthStatus}
            </button>
          ))}
